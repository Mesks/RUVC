...
# use the small version ofRAFT pre-trained pytorch model
class RAFT(nn.Module):
    def __init__(self, use_small:bool=True, mixed_precision:bool=False, dropout:float=0.0, use_dehaze:bool=False):
    ...
    def __freeze_bn(self):
        for m in self.modules():
            if isinstance(m, nn.BatchNorm2d):
                m.eval()

    def __initialize_flow(self, img):
    ...
    def __upsample_flow(self, flow, mask):
    ...
    def __compute_optical_flow(self, image1, image2, iters=12, flow_init=None, upsample=True, test_mode=False):
        ...
        coords0, coords1 = self.__initialize_flow(image1)
        ...
        for itr in range(iters):
            ...
            if up_mask is None:
                ...
                flow_up = self.__upsample_flow(coords1 - coords0, up_mask)
    ...
    def forward(self, reference_frame, frames):
        flow = []
        if self.use_dehaze:
            reference_frame = dehaze.video_dehaze(reference_frame)
            frames          = dehaze.video_dehaze(frames)
        
        padder = InputPadder(reference_frame.shape)
            
        for i in range(frames.shape[1]//3):
            frame              = frames[:, i*3:(i+1)*3, :, :]
            p_frame1, p_frame2 = padder.pad(reference_frame, frame)
            flow_low, flow_up  = self.__compute_optical_flow(p_frame1, p_frame2, iters=20, test_mode=True)
            flow.append(padder.unpad(flow_up))
    ...
    def flow_to_image(self, flow_uv, clip_flow=None, convert_to_bgr=False):
        """
