...
## crf_values:          CRF values for x265 or QP values for vvenc.
  ...
  if [ -d "$data_path" ]; then
    for crf in ${crf_values[@]}; do
      if [ "$frame_number" == "" ]; then
        python -u testVVC_RUVC.py --testdata $data_path --model $model_path --GPU_index $GPU_index --use_surrogate 0 --codec $codec --x265_CRF $crf --prefix $prefix --useLiteRUVC $useLiteRUVC --rescaling_times 2 --show_each_frame $save_intremediate --reference_step $reference_step 2>&1 | tee -a $log_path
      else
        python -u testVVC_RUVC.py --testdata $data_path --model $model_path --GPU_index $GPU_index --use_surrogate 0 --codec $codec --x265_CRF $crf --prefix $prefix --useLiteRUVC $useLiteRUVC --frame_number $frame_number --rescaling_times 2 --show_each_frame $save_intremediate --reference_step $reference_step 2>&1 | tee -a $log_path
      fi
    done
  else
