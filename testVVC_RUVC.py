...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        ## Encode&Decode stage
        decode_video, file_size, quality_value = codec(encode_video)
        ...
        HR_reconstruction, LF_RE, predicted_HF \
                             = rescaling_model(HR_video=padded_reference, LR_video=padded_LF_video, reverse=True, quantization_parameter=codec.normalize_quality(quality_value))
    ...
    logged_quality = quality_value if test_config.codec == 'vvenc' else test_config.x265_CRF
    if test_config.codec == 'vvenc':
        quality_summary = f'Codec: VVC, QP: {int(logged_quality)}'
    else:
        quality_summary = f'CRF: {logged_quality}'
    print(f"Total Video: ({quality_summary})\n\t{'PSNR':<8}= {test_psnr:.6f}\n\t{'MSSSIM':<8}= {test_msssim:.6f}\n\t{'SSIM':<8}= {test_ssim:.6f}\n\t{'LPIPS':<8}= {test_lpips:.6f}\n\t{'bpp':<8}= {test_bpp:.6f}\n\t{'bitrate':<8}= {test_bitrate:.6f}\n\t{'runtime':<8}= {test_runtime:.6f}\n")
    with open(test_config.result_path, 'a') as csvfile:
        writer = csvfile.write(f"{test_config.testdata.split('/')[-1]},{logged_quality},{test_psnr:.6f},{test_msssim:.6f},{test_ssim:.6f},{test_lpips:.6f},{test_bpp:.6f},{test_bitrate:.6f},{test_runtime:.6f},\n")
